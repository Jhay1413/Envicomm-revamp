<template>
    <div class="px-[12px] md:px-[36px] xl:px-0 mt-[70px] lg:mt-[128px]">
        <div class="text-center mb-[30px] md:mb-[90px]">
            <h2
                class="font-bold mx-auto text-[35px] leading-[44px] md:text-[46px] md:leading-[52px] lg:text-heading-1 text-gray-900 mb-5 md:mb-[30px] md:w-[23ch]"
            >
                Environmental Compliance Certificate
            </h2>
            <p class="text-quote md:text-lead-lg text-gray-600 mx-auto md:w-[56ch]">
                (Technical & Non-Technical) - This certificate allows a proposed project to proceed
                to the next stage of project planning.
            </p>
        </div>
        <div class="mx-auto lg:flex gap-[30px] max-w-[1190px]">
            <div
                class="text-center rounded-2xl p-5 max-h-[300px] md:max-h-[450px] lg:max-h-[380px] xl:max-h-[455px] md:p-14 mb-[150px] md:mb-[300px] lg:mb-0 transition-all duration-300 hover:translate-y-[-3px] bg-bg-2"
            >
                <h3 class="font-bold text-[20px] leading-[26px] md:text-heading-4 mb-[18px]">
                    Architectural Plans
                </h3>
                <p class="text-md text-gray-600 mb-[22px] lg:mx-16">
                    blueprints or construction drawings, are detailed documents created by
                    architects and other design professionals .
                </p>
                <NuxtImg
                    class="p-5 rounded-2xl object-cover mx-auto w-full bg-[#EBECF4]"
                    src="/assets/images/thumbnail-8.png"
                    alt="Feature image"
                />
            </div>
            <div
                class="text-center rounded-2xl p-5 max-h-[300px] md:max-h-[450px] lg:max-h-[380px] xl:max-h-[455px] md:p-14 mb-[150px] md:mb-[300px] lg:mb-0 transition-all duration-300 hover:translate-y-[-3px] bg-bg-2"
            >
                <h3 class="font-bold text-[20px] leading-[26px] md:text-heading-4 mb-[18px]">
                    Area Clearance
                </h3>
                <p class="text-md text-gray-600 mb-[22px] lg:mx-16">
                    Community Based Forest Management Agreement (CBFMA) - This is a production
                    sharing agreement .
                </p>
                <NuxtImg
                    class="p-5 rounded-2xl object-cover mx-auto w-full bg-[#EBECF4]"
                    src="/assets/images/thumbnail-9.png"
                    alt="Feature image"
                />
            </div>
        </div>
    </div>
</template>
