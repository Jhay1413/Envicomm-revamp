<template>
    ...
    <div class="px-[12px] md:px-[36px] xl:px-0 mt-[70px] lg:mt-[139px]">
        ...
        <div
            class="input-shadow bg-white p-5 rounded-[36px] lg:py-[72px] lg:px-[66px] lg:flex lg:gap-[38px] mb-[80px]"
        >
            <div
                class="bg-green-900 py-10 px-5 rounded-2xl flex-1 lg:py-10 lg:px-12 mb-[30px] lg:mb-0"
            >
                <div
                    class="inline-block bg-green-500 py-1 bill-monthly bill px-[6px] rounded-[28px] mb-[55px]"
                    @click="handleToggle"
                >
                    <span
                        class="inline-block text-text text-white py-2 cursor-pointer btn-text-monthly px-[21px] rounded-[24px]"
                        :class="{ ' bg-green-900': !isToggled }"
                        >Monthly</span
                    >
                    <span
                        class="inline-block text-text text-white py-2 cursor-pointer btn-text-yearly px-[21px] rounded-[24px]"
                        :class="{ ' bg-green-900': isToggled }"
                        >Yearly</span
                    >
                </div>
            ...
            <div class="flex flex-col gap-6 flex-1 package">
                <div
                    class="border border-gray-400 rounded-2xl flex items-center justify-between cursor-pointer transition-all duration-300 p-[25px] bill-monthly block-price-item active"
                >
                    ...
                    <div>
                        ...
                        ><span class="text-sm font-bold text-gray-400 price-length ml-[2px]"
                            >/<template v-if="isToggled">year</template
                            ><template v-else>month</template></span
                        >
                    </div>
                </div>
                <div
                    class="border border-gray-400 rounded-2xl flex items-center justify-between cursor-pointer transition-all duration-300 p-[25px] bill-monthly block-price-item"
                >
                    <div class="flex items-center gap-5 lg:gap-[35px]">
                        <div
                            class="block-price-icon w-[26px] h-[26px] bg-[url(/assets/images/icons/icon-checkbox.svg)]"
        ...
        <div class="mx-auto flex items-center justify-center flex-col gap-5 md:flex-row">
            <button type="button">
                <NuxtLink
