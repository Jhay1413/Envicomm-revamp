<template>
  <!-- hero section  -->
  <section
    class="bg-[url('/assets/images/main-imgs/what-we-do/application-img.webp')] bg-cover bg-left md:bg-top min-h-dvh py-20 w-full px-10 md:px-20 flex items-center"
    id="top"
  >
    <div class="md:max-w-[45%] text-white">
      <!-- text 1 -->
      <h1 class="text-2xl md:text-5xl 4k:text-6xl mb-5">
        Certificate of Non-Overlap (CNO)
      </h1>
      <p class="text-lg 4k:text-3xl">
        DENR Certificate of Non-Overlap confirms a proposed project area does
        not infringe on ancestral domains or protected areas.
      </p>
      <div class="flex gap-2 mt-5 w-full flex-wrap">
        <NuxtLink
          to="#tsd-new"
          class="4k:text-2xl border-2 border-white transition-all duration-200 text-white py-2 px-5 rounded-lg uppercase"
        >
          New
        </NuxtLink>
      </div>
    </div>
  </section>
  <!-- body  -->
  <section class="py-10 md:py-20">
    <div class="container">
      <!-- section 1  -->
      <section id="tsd-new" class="py-10">
        <h2
          class="text-2xl md:text-4xl font-regular text-center mb-5 uppercase"
        >
          Certificate of Non-Overlap (CNO)
        </h2>

        <h4
          class="uppercase bg-[#547326] py-1 text-xl md:text-2xl 4k:text-3xl text-center rounded text-white font-semibold mb-10"
        >
          scope OF WORK - NEW
        </h4>
        <h5 class="text-lg md:text-xl mb-10 font-semibold">
          RESPONSIBILITIES OF THE CONSULTANT
        </h5>
        <ul class="4k:text-2xl list-disc list-inside mb-10 ml-[19px]">
          <li class="mb-2">
            Facilitate the preparation of the official Letter of Intent to
            secure the CNO
          </li>
          <li class="mb-2">
            Assist in obtaining a CENRO Endorsement, which is a prerequisite for
            the NCIP application
          </li>
          <li class="mb-2">
            Secure a Barangay Certificate confirming the absence of indigenous
            peoples in the area
          </li>
          <li class="mb-2">
            Submit the complete application package to the relevant NCIP office
            and manage all follow-up correspondence
          </li>
          <li class="mb-2">
            Represent the proponent during any field-based investigation (FBI)
            required by the NCIP
          </li>
          <li class="mb-2">
            Ensure the successful issuance of the CNO by addressing any
            additional requirements from the NCIP case handle
          </li>
        </ul>

        <h4
          class="uppercase bg-[#547326] py-1 text-xl md:text-2xl 4k:text-3xl text-center rounded text-white font-semibold mb-10"
        >
          REQUIREMENTS - nEW
        </h4>
        <h5 class="text-lg md:text-xl mb-10 font-semibold">
          REQUIREMENT FROM PROPONENT
        </h5>
        <ul class="4k:text-2xl list-disc list-inside mb-10 ml-[19px]">
          <li class="mb-2">CENRO Endorsement for securing CNO</li>
          <li class="mb-2">Letter of Intent in securing for CNO</li>
          <li class="mb-2">
            Photocopy/Scanned Copy of Government ID of Authorized Representative
          </li>
          <li class="mb-2">
            Barangay Certificate stating that there are no existing indigenous
            people in the area
          </li>
          <li class="mb-2">4 copies of IDMP with attached Sketch Plan</li>
        </ul>

        <div class="flex flex-col gap-3 items-start justify-center">
          <NuxtLink
            to="#"
            class="flex-1 4k:text-2xl bg-[#547326] transition-all duration-200 text-white py-2 px-2 md:px-5 rounded text-center inline-block w-fit mx-auto"
          >
            Download PDF
          </NuxtLink>
          <NuxtLink
            to="#top"
            class="flex-1 4k:text-2xl border-2 border-[#547326] transition-all duration-200 text-[#547326] py-2 px-5 rounded"
          >
            Back to top
          </NuxtLink>
        </div>
      </section>
    </div>
  </section>
</template>

<script setup>
import { useSeo } from "~/composables/useSeo";
useSeo({
  title: "CNO - Certificate of Non-Overlap for Ancestral Domain Claims",
  description:
    "Processing of CNO (Certificate of Non-Overlap) with the NCIP, certifying that your project area does not overlap with any Ancestral Domain Claims.",
  keywords:
    "CNO, Certificate of Non-Overlap, Ancestral Domain Claims, NCIP clearance, land use clearance",
});
</script>

<style lang="scss" scoped></style>
